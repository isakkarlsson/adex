...
class CodePairs:
    ...
    def disproportionality(self, method=prr, min_a=5):
        ...
        value = {}
        a_value = {}
        x_count = frame.drug_distribution[code]
        for drug, count in pairs.iterrows():
            y_count = frame.diagnos_distribution[drug]
            a = count[0]
            if a <= min_a:
                continue
...
def code_pairs(frame, code, delta=timedelta(days=7), pad=timedelta(days=0)):
    ...
    diagnoses = frame.drugs.reset_index().set_index(["Patient", "Date"])
    drugs = frame.diagnoses.reset_index().set_index(["Patient", "Date"])
